<template>
  <div
    class="overlay"
    @click="$parent.$emit('closeAddForm', 'addOrganization')"
  >
...
export default {
  ...
  methods: {
    ...
    recForms() {
      if (this.id) {
        if (this.id) {
          let organisation = this.organisations.filter(
            (item) => item.id === this.id
          );
          this.id = organisation[0].id;
          this.form.name = organisation[0].name;
          this.form.reserves1 = organisation[0].reserv1_id;
          this.form.reserves2 = organisation[0].reserv2_id;
        }
      }
    },
